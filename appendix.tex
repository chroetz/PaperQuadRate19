...
\begin{proof}
	...
	By \assu{ass:wquad}{Weak Quadruple} it holds
	\begin{align*}
		&
...
\begin{proof}
	...
	Let $(Z_1\pr, \dots, Z_n\pr)$ be an independent copy of $(Z_1, \dots, Z_n)$, where $(Y_1\pr, \dots, Y_n\pr)$ is an independent copy of $(Y_1, \dots, Y_n)$.
	By \assu{ass:squad}{Strong Quadruple} it holds
	\begin{align*}
			&
		n^2 \br{Z_i(q)-Z_i(p)-Z_i\pr(q)+Z_i\pr(p)}^2 
		\\&= 
...
\begin{proof}[of \autoref{thm:abstr_weak_strong}]
	...
	By \assu{ass:se}{Small Entropy} and \autoref{lmm:entropyuse} there is $c_\beta>0$ such that for $n\in\N$ large enough, it holds 
	\begin{equation*}
...
\begin{figure}
	...
	\end{tabular}}\\
	\fbox{\begin{tikzpicture}[every path/.style={thick}, baseline=(current bounding box.center)]
